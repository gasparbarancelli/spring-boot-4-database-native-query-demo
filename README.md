...
Abaixo estão exemplos de comandos cURL para testar os principais endpoints da aplicação:
...
### 2. Buscar vendas por ID do cliente
```bash
curl -X GET http://localhost:8080/sales/customer/1
```

### 3. Buscar vendas filtradas (POST com JSON)
```bash
...
### 4. Buscar vendas dinâmicas
```bash
curl -X GET http://localhost:8080/sales/dynamic
```

### 5. Buscar vendas agregadas por cliente
```bash
curl -X GET http://localhost:8080/sales/customers
```

### 6. Buscar vendas agregadas por cliente (paginado)
```bash
